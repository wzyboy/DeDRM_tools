...
from .misc import (list_symbols, natural_sort_key, type_name)
...
class YJContainer(object):
    ...
    def __init__(self, log, book, datafile=None, fragments=None):
        ...
        self.fragments = fragments or YJFragmentList()
...
class YJFragmentKey(IonAnnots):
    ...
    def __cmp__(self, other):
        if isinstance(other, YJFragment):
            return cmp(self, other.annotations)

        if isinstance(other, YJFragmentKey):
            return cmp(self.sort_key(), other.sort_key())

        raise Exception("YJFragmentKey __cmp__: comparing with %s" % type_name(other))

    def sort_key(self):
        return (PREFERED_FRAGMENT_TYPE_ORDER.index(self.ftype) if self.ftype in PREFERED_FRAGMENT_TYPE_ORDER else
                    len(PREFERED_FRAGMENT_TYPE_ORDER), natural_sort_key(unicode(self.fid)))
...
class YJFragment(IonAnnotation):
    ...
    def __cmp__(self, other):
        if isinstance(other, YJFragment):
            return cmp(self.annotations, other.annotations)

        if isinstance(other, YJFragmentKey):
            return cmp(self.annotations, other)

        raise Exception("YJFragment __cmp__: comparing with %s" % type_name(other))
...
class YJFragmentList(IonList):
    ...
    def remove(self, value):
        if not self.discard(value):
            raise KeyError("YJFragmentList remove, item is missing: %s" % unicode(value))
        ...
        for fragment in self[:]:
            if fragment.ftype in RAW_FRAGMENT_TYPES:
                ...
                if omit_large_blobs and ion_type(fragment.value) is IonBLOB and fragment.value.is_large():
                    fragment = YJFragment(ftype=fragment.ftype, fid=fragment.fid,
                            value="*** %d byte BLOB omitted ***" % len(fragment.value))
