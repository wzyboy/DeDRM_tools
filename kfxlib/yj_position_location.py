...
from .ion import (ion_type, IonAnnotation, IonInt, IonList, IonSExp, IonString, IonStruct, IonSymbol,
            IS, unannotated)
from .misc import (list_symbols, truncate_list)
...
class ContentChunk(object):
    def __init__(self, pid, eid, eid_offset, length, section_name, match_zero_len=False):
        ...
        if pid < 0 or (isinstance(eid, int) and eid <= 0) or eid_offset < 0 or length < 0:
            raise Exception("bad ContentChunk: %s" % unicode(self))

    def __eq__(self, other):

        if (isinstance(other.eid, type(self.eid)) and self.eid == other.eid and self.eid_offset == other.eid_offset and
    ...
    def __unicode__(self):
        return "pid=%s eid=%s%s len=%s%s sect=%s" % (self.pid, self.eid,
                    "+" + unicode(self.eid_offset) if self.eid_offset else "", self.length,
...
class ConditionalTemplate(object):
    ...
    def __unicode__(self):
        if self.oper not in ["$298", "$299"]:
            return "%s%s+%s (%s)" % (self.oper, self.end_eid, self.end_eid_offset, " ".join([unicode(x.eid) for x in self.pos_info]))

        return "%s+%s%s%s+%s(%s)" % (self.start_eid, self.start_eid_offset, self.oper, self.end_eid, self.end_eid_offset,
                    " ".join([unicode(x.eid) for x in self.pos_info]))
...
class BookPosLoc(object):
    def check_position_and_location_maps(self):
        ...
        if section_lengths and not self.is_sample:
            ...
            if not (reflow_section_size == reflow_section_size_calculated or reflow_section_size == reflow_section_size_calculated+1):
    ...
    def collect_content_position_info(self, section_names):

        is_conditional_structure = self.get_feature_value("yj.conditional_structure") is not None
        eid_section = {}
        eid_start_pos = {}
        pos_info = []
        section_pos_info = []
        ...
        is_kpr3_21 = (self.get_metadata_value("file_creator", category="kindle_audit_metadata", default="") == "KPR" and
                    self.get_metadata_value("creator_version", category="kindle_audit_metadata", default="") == "3.21.0")
        ...
        def collect_section_position_info(section_name):
            ...
            def extract_position_data(data, current_eid, content_key, list_index, list_max, advance):
                ...
                if data_type is IonAnnotation:
                    ...
                    for i,fc in enumerate(data):
                        if content_key == "$146" and self.is_kpf_prepub:
                    ...
                    if content_key != "$259":
                        ...
                        if eid is not None:
                            ...
                            if current_eid in eid_section:
                                self.log.error("duplicate eid %s: sections %s and %s" % (current_eid, eid_section[current_eid], section_name))
                    ...
                    typ = data.get("$159")
                    if (typ in ["$270", "$271"] and (parent_eid is not None and content_key == "$146" and
                                data.get("$601") == "$283") and list_index is not None):
                    ...
                    for fk,fv in data.items():
                        if fk == "$141":
                                for pt in fv:
                                    extract_position_data(pt, current_eid, fk, None, None, advance)

                        elif fk == "$145" and ion_type(fv) is IonStruct:
                            content = self.fragments[YJFragmentKey(ftype="$145", fid=fv["$4"])].value["$146"][fv["$403"]]
                            have_content(current_eid, len(content), advance)

                        elif fk == "$146":
                            if typ == "$274":
                                extract_position_data(fv, current_eid, typ, None, None, advance)
                            elif typ == "$272":
                                extract_position_data(fv, None, fk, None, None, False)

                            else:

                                have_content(current_eid, 1,
                                    advance and data.get("$615") not in ["$688", "$689"])

                                extract_position_data(fv, current_eid, fk, None, None, advance)

                        elif fk == "$176" and content_key != "$259":
                            have_content(current_eid, 1, advance)
                            if is_conditional_structure:
                        ...
                        elif fk in ["$584", "$683", "$250",
                                    "yj.dictionary.term", "yj.dictionary.unnormalized_term"]:
                            pass

                        elif ion_type(fv) != IonString or fk == "$145":
                            extract_position_data(fv, current_eid, fk, None, None, advance)
                    ...
                    have_content(current_eid, length, advance, allow_zero=list_index < list_max)
            ...
            for ci in eid_cond_info:
                if ci.pos_info:
                    self.log.warning("left over conditional template info: %s" % ci)
    ...
    def collect_position_map_info(self, section_names):
        pos_info = []
        eid_start_pos = {}
        prev_eid_offset = {}
        eid_section = {}
                ...
                if i > 0:
                    if section_name is not None:
                        if eid in eid_section and eid_section[eid] != section_name:
                            self.log.error("section_position_id_map eid %s expected in section %s found in %s" % (eid,
                    ...
                    if is_conditional_structure or has_mathml:
                    ...
                    pos_info.append(ContentChunk(pid + section_start_pid, eid, eid_offset, next_pid - pid,
        ...
        if self.is_dictionary or self.is_kpf_prepub:
            ...
            if fragment is not None:
                ...
                section_start_pid = 0
                for section_name in section_names:
                    spim_fragment = self.fragments[YJFragmentKey(ftype="$609", fid=unicode(section_name) + "-spm")]
            ...
            if esm_eids:
                ...
                if esm_missing:
                    self.log.warning("yj.eidhash_eid_section_map has %d eids, %d missing: %s" % (len(esm_eids), len(esm_missing),
                            ", ".join(truncate_list(["%s/%s" % xx for xx in sorted(list(esm_missing))]))))

                spim_missing = esm_eids - spim_eids
                if spim_missing:
                    self.log.warning("section_position_id_map has %d eids, %d missing: %s" % (len(spim_eids), len(spim_missing),
    ...
    def verify_position_info(self, section_names, content_pos_info, map_pos_info):
        class PosData(object):
            ...
            def advance(self, extra=False):
                ...
                if chunk.pid != self.next_pid:
                    report.report("position_id %s expected pid %d at idx=%d %s" % (self.name, self.next_pid, self.index, unicode(chunk)))

                if extra:
                    report.report("position_id %s extra at idx=%d %s" % (self.name, self.index, unicode(chunk)))
    ...
    def create_position_map(self, section_names, pos_info):
    ...
    def collect_location_map_info(self, pos_info):
        ...
        if not (self.is_dictionary or self.is_kpf_prepub):
            positionMaps_fc = self.get_feature_value("kfxgen.positionMaps", namespace="format_capabilities")
            if (positionMaps_fc == 2 and not self.is_textbook) is not has_yj_location_pid_map:
                self.log.error("FC kfxgen.positionMaps=%s textbook=%s with yj.location_pid_map=%s" % (
                        positionMaps_fc, self.is_textbook, has_yj_location_pid_map))
    ...
    def create_approximate_page_list(self, desired_num_pages):
        ...
        reading_order_name,section_names = self.get_default_reading_order()
        if reading_order_name is None:
            self.log.error("Cannot create page numbers - Failed to locate default reading order")
            return
                ...
                if book_navigation["$178"] == reading_order_name:
                    ...
                    for i,nav_container in enumerate(nav_containers):
                        ...
                        if nav_container.get("$235", None) == "$237":
                            ...
                            nav_container_name = unicode(nav_container.get("$239"))
                            real_num_pages = len(nav_container["$247"])
                            self.log.info("A list of %d %s pages is already present with %s pages desired" % (real_num_pages,
                                    "approximate" if nav_container_name == APPROXIMATE_PAGE_LIST else "real",
                                    unicode(desired_num_pages) if desired_num_pages else "auto"))
        ...
        pos_info = self.collect_position_map_info(section_names)

        page_template_eids = set()
        for section_name in section_names:
            fragment = self.fragments.get(ftype="$260", fid=section_name)
            if fragment is not None:
                self.walk_fragment(fragment, set(), set(), page_template_eids)
        ...
        if self.is_fixed_layout:
            ...
            pages,new_section_page_count = self.determine_approximate_pages(pos_info, page_template_eids, section_names[0], 999999, True)
            self.log.info("Created %d fixed layout page numbers" % len(pages))

        elif desired_num_pages == 0:

            pages,new_section_page_count = self.determine_approximate_pages(pos_info, page_template_eids, section_names[0], TYPICAL_POSITIONS_PER_PAGE)
